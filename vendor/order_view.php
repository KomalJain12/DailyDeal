...
/*$q="select o.*,m.* from order_master o,order_detail m where o.order_id=m.order_id and vid=$vid ORDER BY order_detail_id desc;";*/
                               $vid=$_SESSION['id'];
                                $f=mysqli_query($conn,"select * from order_detail where pid in (select pid from product_detail where vid=$vid) order by order_detail_id DESC");
...
<td>
<a href="order_status.php?order_detail_id=<?php echo $row["order_detail_id"]; ?>" onclick='return accept ()'><button type="button" class="btn btn-primary" name="delete"> Accept </button></a></td>
<td>
<a href="order_dispatch.php?order_detail_id=<?php echo $row["order_detail_id"]; ?>" onclick='return dispatch ()'><button type="button" class="btn btn-primary" name="delete"> Dispatch </button></a></td>
<td>
<a href="order_cancel.php?order_detail_id=<?php echo $row["order_detail_id"]; ?>" onclick='return cancel ()'><button type="button" class="btn btn-primary" name="delete"> Cancel </button></a>
</td>
                                        </tr>
...
function accept()
{
    return confirm('Are you Sure to Accept Order ?');
}
</script>
<script>
function dispatch()
