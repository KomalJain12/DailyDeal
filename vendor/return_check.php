...
/*$q="select o.*,m.* from order_master o,order_detail m where o.order_id=m.order_id and vid=$vid ORDER BY order_detail_id desc;";*/
                               $vid=$_SESSION['id'];
                                $f=mysqli_query($conn,"select * from order_return_master where status=0 and pid in (select pid from product_detail where vid=$vid) order by return_id DESC");
...
<td>
<a href="return_status.php?return_id=<?php echo $row["return_id"]; ?>" ><button type="button" class="btn btn-primary" name="delete"> Accept </button></a></td>
<td>
